...
*Full technical report. For the five-minute version, see the [portfolio summary](https://github.com/AthertHa-FinOps/finops-cost-allocation-gap-summary).*
...
**The forensic layer is captured evidence.** The EC2 instance I investigated, `i-0c9cfb67280fe44ee`, was really provisioned in this account, in `us-
...
in the AWS Pricing Calculator, built a CUR-shaped dataset from that baseline, and loaded it into Athena. The dollar figures throughout Case Study 1 are 
scenario values derived from that baseline, not billed spend: $315 total, $55 unallocated, a 17% allocation gap. Construction method is documented in 
Appendix C.
...
**Two resource-provenance notes.** The EC2 resource exists in both layers. The instance is real. The cost attached to it is scenario data. The S3 
resources (`bucket-logs-arch`, `bucket-temp-data`) exist only in the scenario layer; no corresponding buckets were provisioned. And where the scenario 
assigns a larger instance type than the sandbox actually ran, that is the scenario modelling the resource at representative production sizing. The 
captured evidence is authoritative for what was really provisioned.
...
metadata, but as an audit problem. Reconcile the financial record against operational evidence, find where they diverge, work out why the control 
failed, then recommend something that reduces recurrence. Different technology, same discipline.
...
The standard I assessed against was `Environment`, `Project`, and `Owner` on all billable resources. I defined it for this assessment, and §3.2 shows a 
compliant resource that meets it. In a real organisation this standard would pre-exist; here it was established so the assessment had something to 
measure against.
...
Service totals reconcile against the aggregate, so the billing is internally consistent. That rules out the simplest explanation and pushes the 
investigation down to the resource level.
...
it entered the environment without passing through any tagging standard at all. That is a process failure rather than an individual one. That 
...
this needs a provisioning control or a cleanup process. Those are very different remediations.
...
***[Captured]** Real CloudTrail record. Root principal, MFA-authenticated, `us-east-2`, `eventTime` 2026-01-22T19:00:13Z, and a Chrome UserAgent 
confirming a console launch rather than CLI or SDK.*
...
***[Captured]** `i-0c9cfb67280fe44ee`, `us-east-2a`, launched Thursday 22 January 2026 at 14:00:13 GMT-0500. CloudTrail records the same instant as 
...
30 minutes preceding the `RunInstances` call; none returned. This was not a role assumed and then used. There was no attributable identity chain at any 
layer. The consequences are developed in §3.3.
...
layer. In a real enterprise, root is typically locked via SCP. The production equivalents are a broad-permission developer role launching via console,
a federated SSO user working outside an IaC pipeline, or a break-glass role used outside a change window. The forensic technique applies identically to
all three: UserAgent analysis plus `tagSpecificationSet` inspection.
...
***[Executed]** Real Athena query against the scenario dataset, using `COALESCE` on the environment tag column, `UNION ALL` for the total row, and 
`CASE`-based ordering. 0.27 KB scanned.*
...
Partial tagging produces the same Finance-invisible result as no tagging at all. A chargeback query filtering on environment excludes these rows 
entirely.
...
The tag activation dependency is the quietest of the three and worth naming explicitly. If `Environment`, `Project`, and `Owner` had not already been 
activated as Cost Allocation Tags, the CUR columns would not exist. The investigation would have returned incomplete results while appearing to 
...
Against that baseline, the investigated resource carried only `Name`. That tag makes a resource findable in the console but contributes nothing to cost 
allocation. `Name` and cost allocation tags are independent, and a resource can be perfectly visible to engineering while remaining invisible to 
Finance.
...
The alert payload carries the missing tags, the IAM principal ARN, the account, and the region. Those are the fields Finance needs to open a query, and 
the fields required to route the alert to an owner.
...
*(Also identified: an unused `boto3` import in the deployed function. No functional impact, and it is removed in the corrected version.)*
...
services, and exception management. The SCP alone does not do it.
...
3. Identify approved exceptions: AWS-managed services, automation roles, and break-glass workflows.
...
**What a sandbox does not surface.** Regional deployment, since EventBridge rules are regional. The defect above is the sandbox-scale version of a 
problem that multiplies across an Organization. Lambda concurrency limits under bursty provisioning, addressed with an SQS buffer and DLQ. CUR 
partition management past 100+ accounts. And tag-key normalisation across accounts that have not standardised on `Environment` versus `env` versus 
`environment`. Each variant produces a separate NULL pattern in CUR until Tag Policies fix the schema.
...
see or charge back. That is $6M annually. At that scale it stops being a reporting gap and becomes a planning failure: budget models are wrong, team 
...
- **Financial data has to be treated as operational telemetry.** Without near-real-time visibility, allocation failures sit undetected until month-end close.
- **Root causes at this scale are rarely a single failure.** The provisioning gap, the tag activation dependency, and the identity gap are three separate problems that compounded.
...
control silently stop working. That is the same question Appendix B asks about the pipeline as a whole.
...
*The following is a modelled exercise, not an investigation. There is no underlying account, billing data, or log evidence. The figures are constructed 
to work through commitment-sizing reasoning at a scale the sandbox cannot produce. It is presented separately from Case Study 1 for that reason.*

## Cross-Account Cost Spike and Commitment Coverage

***[Modelled] throughout.***

**Scenario.** A weekly EC2 cost review across three linked accounts under consolidated billing shows spend rising from a four-week baseline of roughly 
$54,100 per week to $72,730. That is an increase of $18,630, or 34% week over week, five days before month-end.
...
$18,400 of the $18,630 increase sits in Account B, in compute-optimised families. That is the shape of a batch workload rather than organic growth.
...
dev account with no budget guardrail. Spend reaches billing without Finance visibility until a consolidated threshold fires five days later.
...
**One structural caveat.** Shared infrastructure like VPCs, Transit Gateways, and NAT Gateways often has no single owner. Any chargeback split for 
those has to be agreed before tagging is meaningful at all, because there is no correct `Owner` value to enforce. Tagging cannot resolve an unanswered 
ownership question; it can only record the answer once someone makes the decision.
