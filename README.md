...
> **30-second read:** Finance found a 17% AWS cost allocation gap. Forensic investigation using CUR and Athena SQL traced it to console-launched resources missing required tags at creation. I built an event-driven governance pipeline (CloudTrail → EventBridge → Lambda) that cut detection latency from 30 days to under 5 minutes. Allocation coverage is back to 100%.

---

## Project Summary

I investigated a 17% AWS cost allocation gap using CUR and Athena SQL. CloudTrail forensics traced the issue to console-launched resources that were missing required tags at creation, confirmed by the IAM principal, UserAgent string, and tagSpecificationSet evidence. I then built an event-driven governance pipeline (CloudTrail → EventBridge → Lambda) to detect non-compliant resources in near-real-time. Allocation coverage is restored to 100% and detection latency dropped from roughly 30 days to under 5 minutes.

> **This investigation pattern scales directly to enterprise environments.** The CUR query structure, CloudTrail forensic chain, and EventBridge governance pipeline work the same way whether the environment has 1 account or 200. Multi-account architecture, AWS Organizations SCP progression, Lambda concurrency considerations, and CUR partition strategy for $2M to $10M per month in spend are all covered in the [Enterprise Context](#enterprise-context-and-scale) section below.
...
| Finance reconciliation effort | ~4 hrs/month | Eliminated |
...
**Pipeline flow:** EC2 or S3 resource creation triggers CloudTrail to capture the API call. EventBridge matches RunInstances and CreateBucket events. Lambda validates Environment, Project, and Owner tags. Slack alerts fire to the Finance channel and the resource owner simultaneously.
...
| **Result** | Allocation coverage restored to 100%. MTTD reduced from 30 days to under 5 minutes. Finance monthly reconciliation effort eliminated. |
...
| 3. CloudTrail Forensics | Identify creation source and method | CloudTrail event history | Console launch confirmed, tags absent at creation |
...
During monthly reconciliation, Finance flagged a discrepancy. Total AWS spend was $315. Spend visible with required tags was $262. That left $53 unallocated, roughly 17% of the total. The invoice was accurate. The failure was in cost attribution, not cost generation.
...
The first assumption to test was a billing error. If the invoice was wrong, the allocation gap was a Finance reporting artifact and not an infrastructure problem. I validated service totals in Cost Explorer, reconciled them against CUR via Athena, and confirmed the invoice totals matched exactly.
...
The Environment, Project, and Owner tags were activated as Cost Allocation Tags in the AWS Billing and Cost Management console before this investigation started. That activation step is commonly missed in new AWS environments and has to be completed before tag-based CUR analysis is possible.
...
> **Note on the $40 figure:** The resource-level CUR query shows $40. The service-level breakdown in Phase 4 shows approximately $38 for the EC2 gap. This variance comes from partial-month usage and CUR line-item rounding across billing periods. Both figures reconcile to invoice-level totals.
...
I filtered CloudTrail Event history to `RunInstances` and opened the full JSON event record for `i-0c9cfb67280fe44ee`. This is the primary forensic document. It confirms the IAM principal, the launch timestamp, the user agent (console vs. CLI vs. SDK), and the complete tag specification submitted with the API call.
...
Scrolling deeper into the same CloudTrail JSON event record surfaces the `tagSpecificationSet`. Only the `Name` tag (`PROD-WEB-SERVER-01`) was submitted with the API call. `Environment`, `Project`, and `Owner` were not included. This confirms the tags were absent at creation. They were never submitted with the API call and were not applied and later removed.
...
| CloudTrail JSON (top half) | Root with MFA IAM principal, Chrome browser UserAgent confirming console launch, awsRegion us-east-2, creation timestamp 2026-01-22T19:00:13Z |
...
> **CUR isolated the resource. CloudTrail confirmed who created it, how it was created, and that required tags were never submitted.** The Chrome browser UserAgent confirmed console origin and not CLI or SDK. The `tagSpecificationSet` confirmed a provisioning-time enforcement gap with no drift and no tampering. Root cause confirmed.
...
> **Note on root access:** The IAM principal was a Root session with MFA, used intentionally in the sandbox to simulate a worst-case governance bypass scenario. In production environments, root access is restricted and not used for routine infrastructure provisioning. This would be a separate finding in any real security review.
...
EC2 had the largest gap, but S3 confirmed the issue was systemic. The two S3 buckets are represented with partial tags to demonstrate a key compliance principle: **partial tagging produces the same Finance-invisible result as no tagging.** Both buckets are missing the required `Environment` tag. Even with `Project` and `Owner` present, that one missing tag meant $15 per month in storage spend that Finance could not allocate to any cost centre.
...
> **Note on S3 tagging behaviour in production:** The S3 CreateBucket API does not accept tags inline in the creation request the way EC2 does. Tags on S3 buckets are applied via a separate `PutBucketTagging` call after the bucket exists. This means a CreateBucket event in CloudTrail will always show an empty tag list in request parameters, even for buckets that will eventually be tagged correctly. In a production implementation, the EventBridge rule would monitor both `CreateBucket` and `PutBucketTagging` events, or a time-bounded compliance check would verify tag status shortly after bucket creation. This is documented in [What I Would Do Next](#what-i-would-do-next).

> **Lab simulation note:** The actual lab buckets were created with no tags at all. The CUR simulation was deliberately constructed with partial tags (Project and Owner present, Environment absent) to illustrate the compliance principle above. A resource with two of three required tags is just as invisible to Finance reporting as a resource with none.

---

## Reference: Production-Equivalent Unallocated Spend Query

This query replaces the hardcoded Phase 2 lab query when running against a live CUR dataset in production.
...
> **Dialect note:** `DATE_TRUNC` and the `BETWEEN DATE '...'` syntax are Athena/Presto-specific and are not ANSI SQL portable.

The `line_item_usage_account_id` column lets you trace attribution gaps back to the owning account across 50 to 200 accounts in a consolidated Organizations CUR dataset.
...
If tagging depends on humans being perfect, it will eventually fail. This was not a user error. It was a system design gap that needed a system-level response.

I chose EventBridge and Lambda over AWS Config because they provide near-real-time detection. Config rules operate on a minutes-to-hours evaluation cycle and carry Config recorder charges for every resource state change. EventBridge fires within seconds of the API call and avoids those charges at scale.

I prioritized alerting over blocking to preserve engineering velocity. SCP enforcement is planned only after compliance stabilizes above 95%. That reflects a deliberate bias toward fast feedback, low friction, and defensible financial reporting.
...
> **Lab region note and production deployment pattern:** The documented incident occurred in `us-east-2`. The EventBridge rule and Lambda are deployed in `us-east-1`. Amazon EventBridge default event buses are regional, so a production deployment requires the rule in every active region. Two standard patterns handle this:
>
> **Per-region deployment:** Deploy via IaC (Terraform or CloudFormation StackSets). This is the standard approach for organizations with 3 to 5 active regions.
>
> **Central event bus pattern:** Forward regional CloudTrail events to a central EventBridge bus in a dedicated billing or security account. This is the recommended approach for organizations managing 10 or more accounts.
>
> The governance logic is production-ready. Multi-region deployment via IaC is documented in [What I Would Do Next](#what-i-would-do-next).

---

### Step 2: Lambda Validation Logic

Lambda inspects the resource tags in the API event payload and validates that `Environment`, `Project`, and `Owner` exist. The function handles both EC2 and S3 creation events. Non-compliant resources trigger an alert containing the event name, missing tags, IAM principal, account ID, and region.
...
def lambda_handler(event, context):
    ...
    for item in tag_specs:
        tag_list += [t['key'] for t in item.get('tags', [])]  # += for multi-resourceType safety
...
> `boto3` retrieves the Slack webhook URL from AWS Secrets Manager in production (`boto3.client('secretsmanager')`). In this lab, alerts route to CloudWatch Logs via `print()`. The webhook URL must never be hardcoded. The `tag_list +=` pattern rather than `=` ensures tags are captured across multiple `resourceType` entries in a single `RunInstances` call. Alerts fire without blocking deployment, preserving CI/CD velocity. For high-volume burst scenarios, add SQS and DLQ between EventBridge and Lambda to handle concurrency spikes above the default 1,000 concurrent execution limit per account per region.

---

### Step 3: End-to-End Test Execution

I tested the function with a simulated untagged `RunInstances` event. All three required tags were correctly identified as missing and the `FINOPS TAG COMPLIANCE VIOLATION` alert fired correctly.
...
> **Note on Billed Duration vs. execution time:** The function ran in 2.54 ms but AWS billed 259 ms on this invocation. The difference is the 255.90 ms cold start, which is the one-time cost of loading the Python runtime on first invocation. Subsequent warm invocations bill only the 2.54 ms execution time. Cold start has no impact on alert latency since EventBridge invocations are not time-critical at the millisecond level.
...
> **Note on test event values:** The test uses a synthetic IAM principal (`arn:aws:iam::123456789012:user/developer-01`) and placeholder account ID (`123456789012`) rather than reproducing the Root session from the forensic chain. This is intentional since reproducing real Root credentials in a test event is a security anti-pattern. `123456789012` is a well-recognized AWS documentation placeholder. The test validates function logic, not actor identity.
...
- **CUR partition management:** 100 or more accounts at $2M to $10M per month requires partitioning by account ID, service, and billing month. Without it, full-table Athena scans become expensive.
...
|---|---|---|---|
| Tagged spend / Total spend | ~83% | 100% | 95% or above |

---

## What I Would Do Next

**Days 1 to 30: Harden regional coverage**
- Deploy EventBridge rule and Lambda to all active regions via IaC (Terraform or CloudFormation StackSets)
- Add `PutBucketTagging` monitoring to the EventBridge rule pattern for accurate S3 compliance detection
- Implement SQS and DLQ between EventBridge and Lambda for burst hardening

**Days 31 to 60: Establish compliance baseline**
- Centralize CUR across all accounts via AWS Organizations
- Stand up a weekly tag compliance KPI (tagged spend divided by total spend) as a first-class FinOps metric
...
- Run EC2 rightsizing analysis against 2-week CloudWatch utilization windows, which is now possible because attribution is complete
- Deliver showback reporting to Finance and move to a full chargeback model once coverage is stable above 95%
...
| 03 - cloudtrail-forensics-... | Athena Phase 2 CUR query |
...
| 05b-ii - lambda-settings | Lambda runtime: Python 3.12+, x86_64 |
| 05c - lambda-test-result | Test execution: statusCode 200, 2.54 ms, all three tags missing |
...
| Manual reconciliation eliminated | ~4 hours per month |
